import { OwnCapability, PermissionRequestEvent, useCall } from '@stream-io/video-react-native-sdk';
import React, { useEffect, useState } from 'react';
import { Text, Button, ScrollView, StyleSheet, View } from 'react-native';

export const PermissionRequestsPanel = () => {
  const call = useCall();
  const canUpdatePermissions = call?.permissionsContext.hasPermission(OwnCapability.SEND_AUDIO);
    ...
    <ScrollView style={styles.scrollContainer}>
      {speakingRequests.map((request) => (
        <View style={styles.itemContainer} key={request.user.id}>
          ...
          <Button title='Approve' onPress={() => handlePermissionRequest(request, true)} />
          <Button title='Deny' onPress={() => handlePermissionRequest(request, false)} />
        </View>
      ))}
    </ScrollView>
  );
